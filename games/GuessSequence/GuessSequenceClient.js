// Клиентская часть игры - возвращает HTML и функции для управления
class GuessSequenceClient {
    constructor() {
        this.container = null;
        this.gameState = null;
        this.onAction = null;
    ...
    renderGame() {
        ...
        let html = `
            <div class="game-header">
                <h2>🎯 Угадай последовательность</h2>
                <div class="game-mode">Раунд ${state.round + 1}</div>
            </div>
    ...
    renderGameOver() {
        ...
        return `
            <div style="text-align:center;padding:40px;">
                ...
                <div style="margin-top:20px;font-size:14px;color:#999;">Правильная последовательность: ${state.sequence ? state.sequence.map(c => this.getColorName(c)).join(' → ') : '—'}</div>
            </div>
        `;
    }
    ...
    isCurrentPlayer(role) {
        ...
        if (role === 'setter') {
            return state.currentSetter === playerName;
        } else if (role === 'guesser') {
            return state.currentGuesser === playerName;
        }
        return false;
    }

    getPlayerName(playerId) {
        // В реальности нужно получать имя по ID
        const state = this.gameState;
    ...
    addEventListeners() {
            ...
            btn.onclick = () => {
                ...
                if (this.onAction) {
                    if (this.gameState.isSetting) {
                        this.onAction('addColor', { color: color });
                    } else if (this.gameState.isGuessing) {
                        this.onAction('addGuess', { color: color });
                    }
                }
            };
        });
        ...
        container.querySelectorAll('#sequenceDisplay .color-block').forEach(block => {
            block.onclick = () => {
                ...
                if (this.onAction && this.gameState.isSetting) {
                    this.onAction('removeColor', { index: index });
                }
            };
        });
        ...
        container.querySelectorAll('#guessDisplay .color-block').forEach(block => {
            block.onclick = () => {
                ...
                if (this.onAction && this.gameState.isGuessing) {
                    this.onAction('removeGuess', { index: index });
                }
            };
        });
        ...
        if (toggleRemoveBtn) {
            toggleRemoveBtn.onclick = () => {
                if (this.onAction) {
                    this.onAction('toggleRemoveMode', {});
                }
            };
        }
...
if (typeof module !== 'undefined' && module.exports) {
    module.exports = GuessSequenceClient;
}
